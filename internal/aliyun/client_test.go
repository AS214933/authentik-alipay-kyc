...
func (f fakeSDK) InitFaceVerify(*cloudauth.InitFaceVerifyRequest) (*cloudauth.InitFaceVerifyResponse, error) {
	if f.err != nil {
		return nil, f.err
	}
...
func TestInitializeFallsBackAcrossEndpoints(t *testing.T) {
	client := &Client{
		clients: []*endpointClient{
			...
			{endpoint: "secondary", client: fakeSDK{initResp: initResponse("CERT123", "https://aliyun.example/certify")}},
		},
		sceneID:     1000000006,
		productCode: "ID_PRO",
		model:       "MOVE_ACTION",
